...
class Functor1 {
public:
    Functor1(SOS::MemoryView::ReaderBus<BLOCK>& readerBus)
        : _readerBus(readerBus)
        , buffer(ringbufferbus, _readerBus)
    {
        _thread = std::thread { std::mem_fn(&Functor1::operator()),
            this };
    }
    ~Functor1()
    {
