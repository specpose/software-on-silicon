    ...
    namespace Behavior{
        ...
        private SOS::Protocol::SimulationBuffers {
            ...
            SOS::Protocol::SimulationBuffers(in_buffer,out_buffer)
            {
                ...
                SOS::Behavior::PassthruEventController<ProcessingHook>::_intrinsic.getAcknowledgeRef().clear();//INIT: start one-way handshake
            }
            private:
            virtual bool handshake() final {
                if (!SOS::Behavior::PassthruEventController<ProcessingHook>::_intrinsic.getUpdatedRef().test_and_set()){
                    return true;
                }
                return false;
            }
            virtual void handshake_ack() final {SOS::Behavior::PassthruEventController<ProcessingHook>::_intrinsic.getAcknowledgeRef().clear();}
        ...
        private SOS::Protocol::SimulationBuffers {
            ...
            private:
            virtual bool handshake() final {
                if (!SOS::Behavior::PassthruEventController<ProcessingHook>::_intrinsic.getAcknowledgeRef().test_and_set()){
                    return true;
                }
                return false;
            }
            virtual void handshake_ack() final {SOS::Behavior::PassthruEventController<ProcessingHook>::_intrinsic.getUpdatedRef().clear();}
            virtual unsigned char read_byte() final {
                return SOS::Protocol::SimulationBuffers::read_byte();
            }
            virtual void write_byte(unsigned char byte) final {
