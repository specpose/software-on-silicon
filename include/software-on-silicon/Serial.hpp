...
namespace SOS {
    ...
    namespace Protocol {
        ...
        public virtual SOS::Behavior::PassthruEventController<ProcessingHook, DataBus> {//write: 3 bytes in, 4 bytes out; read: 4 bytes in, 3 bytes out
            ...
            private:
            std::size_t& readDestination = std::get<0>(SOS::Behavior::PassthruEventController<ProcessingHook, DataBus>::_foreign.const_cables).getReadDestinationRef();
            std::size_t& writeOrigin = std::get<0>(SOS::Behavior::PassthruEventController<ProcessingHook, DataBus>::_foreign.const_cables).getWriteOriginRef();
