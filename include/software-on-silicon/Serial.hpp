    ...
    namespace Protocol
    {
        ...
        protected:
            ...
            void resend_current_object()
            {
                if (send_lock || writeCount != 0){
                    ...
                    send_lock = false;
                    writeCount = 0;
                    _write_transfer_request(foreign().writeOrigin());
                }
            }
        ...
        private:
            ...
            void _write_transfer_request(unsigned char objectid){
                send_request();
                foreign().writeOrigin() = objectid;
                writeOriginPos = 0;
                std::bitset<8> id;
                write_bits(id);
                std::bitset<8> obj_id = static_cast<unsigned long>(foreign().writeOrigin()); // DANGER: overflow check
                id = id ^ obj_id;
            ...
            bool getFirstSyncObject()
            {
                ...
                for (std::size_t i = 0; i < foreign().descriptors.size() && !gotOne; i++)
                {
                    ...
                    if (receive_lock && i==foreign().readDestination() && !foreign().descriptors[i].synced)
            ...
            void write_object(int &write3plus1)
            {
                ...
                {
                    unsigned char data;
                    data = reinterpret_cast<char *>(foreign().descriptors[foreign().writeOrigin()].obj)[writeOriginPos++];
                    write(data);
                    write3plus1++;
                }
                else
                { // write3plus1==3
                    if (writeOriginPos == foreign().descriptors[foreign().writeOrigin()].obj_size)
                    {
                        foreign().descriptors[foreign().writeOrigin()].synced = true;
                        send_lock = false;
                        writeOriginPos = 0;
                        foreign().descriptors[foreign().writeOrigin()].tx_counter++; // DEBUG
                        // std::cout<<typeid(*this).name();
                        // std::cout<<"$";
                    }
            ...
            void read_object(int &read4minus1)
            {
                ...
                else if (read4minus1 == 3)
                {
                    ...
                    if (readDestinationPos == foreign().descriptors[foreign().readDestination()].obj_size)
                    {
                        //foreign().descriptors[foreign().readDestination()].readLock = false;
                        receive_lock = false;
                        foreign().signal.getUpdatedRef().clear();
                        foreign().descriptors[foreign().readDestination()].rx_counter++; // DEBUG
                    }
                    else
                    {
                        for (std::size_t i = 0; i < 3; i++)
                        {
                            reinterpret_cast<char *>(foreign().descriptors[foreign().readDestination()].obj)[readDestinationPos++] = read3bytes[i];
                        }
                    }
                    read4minus1 = 0;
                }
