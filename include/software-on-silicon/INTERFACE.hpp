...
namespace MemoryView {
    ...
    struct ConstCable : public std::array<const T, N> {
        ConstCable(const std::array<const T, N>& a)
            : std::array<const T, N>(a)
        {
        }
    ...
    template <typename T, size_t N>
    struct TaskCable : public std::array<std::atomic<T>, N> {
        using std::array<std::atomic<T>, N>::array;
        using wire_names = enum class empty : unsigned char { };
        using cable_arithmetic = T;
    };
    // inheritable
    template <typename Category, typename Signal, typename Cables, typename ConstCables>
    struct bus {
...
namespace Behavior {
    ...
    protected:
        template <typename C>
        static std::thread start(C* startme)
        { // ALWAYS requires that startme is derived from this LoopImpl
    ...
    template <typename T, typename S = typename std::enable_if<std::is_base_of<typename SOS::Behavior::SubController, T>::value, T
                              // false,T
                              >::type>
    class Controller {
    public:
    ...
    template <typename S>
    class AsyncController : public Controller<S>, public Loop {
    public:
        AsyncController()
            : Controller<S>()
            , Loop()
