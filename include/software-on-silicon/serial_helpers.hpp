#include <bitset>
#include <cmath>//intel_hls?
template<unsigned int Bytes> constexpr unsigned long long maxUnsignedInt() {
...
template<unsigned int Bytes> constexpr unsigned long long bytearrayToUnsignedNumber(char source[Bytes]){
    unsigned long long result = 0;
    for (int i=0;i<Bytes;i++){
...
template<unsigned int Bytes, unsigned long Bits> constexpr void bitsetToBytearray(unsigned char dest[Bytes],std::bitset<Bits>& source){
...
template<unsigned int Bytes, unsigned long Bits> constexpr void bytearrayToBitset(std::bitset<Bits>& dest,unsigned char source[Bytes]){
