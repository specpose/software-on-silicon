template<typename Current, typename... Args> void write_blink_interleaved(Current current, Args... args) {
    std::cout<<"=";
};
template<typename Current, typename Buffer> void write_blink_interleaved(Current current, const Buffer& buffer){
    std::cout<<"=";
    for (std::size_t sample=0;sample<std::tuple_size<RING_BUFFER::value_type>{};sample++)
        (*current)[sample]=buffer[sample];
}
template<typename Target, typename... Args> void WriteInterleaved(SOS::MemoryView::RingBufferBus<Target>& myBus, Args...) {};
